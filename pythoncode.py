...
class FeatureGenerator(object):
    def __init__(self, dtype, n_jobs=1, chunk_size=None):
        ...
        self.test_files = []
        if self.dtype == 'train':
            self.filename = 's3://subhoms-test/input/data/LANL/LANL_Earthquake_Prediction/train.csv'
            self.total_data = int(629145481 / self.chunk_size)
        else:
            submission = pd.read_csv(
                's3://subhoms-test/input/data/LANL/LANL_Earthquake_Prediction/sample_submission.csv')
            for seg_id in submission.seg_id.values:
                self.test_files.append(
                    (seg_id, 's3://subhoms-test/input/data/LANL/LANL_Earthquake_Prediction/test/' + seg_id + '.csv'))
...
if __name__ == "__main__":
    ...
    # sc = SparkContext("local", "PySpark LANL")
    ...
    train_generate_time = datetime.now();
    print("train_generate_time: " + str(train_generate_time))
    print("train_generate_time - start_time: " + str(train_generate_time - start_time))

    print("writing data to S3")
    training_data.to_csv(
        "s3://subhoms-test/input/data/LANL/LANL_Earthquake_Prediction/features/train_features.csv")

    train_data_to_s3_time = datetime.now();
    print("train_data_to_s3_time: " + str(train_data_to_s3_time))
    print("train_data_to_s3_time - train_generate_time: " + str(train_data_to_s3_time - train_generate_time))

    test_fg = FeatureGenerator(dtype='test', n_jobs=10, chunk_size=150000)
    test_data = test_fg.generate()

    test_generate_time = datetime.now();
    print("test_generate_time: " + str(test_generate_time))
    print("test_generate_time - train_data_to_s3_time: " + str(test_generate_time - train_data_to_s3_time))
    ...
    print("end: " + str(end))
    print("end - test_generate_time: " + str(end - test_generate_time))
    ...
    submission.to_csv("s3://subhoms-test/input/data/LANL/LANL_Earthquake_Prediction/submission.csv", index=False)
